  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-6 rounded-3xl bg-white border border-[#EBE5DF] shadow-xs">
        {loading ? (
          <div className="py-12 text-center text-xs text-[#7A746E]">Loading customer records...</div>
        ) : filteredCustomers.length === 0 ? (
          <div className="py-16 text-center text-sm text-[#7A746E]">No customers found.</div>
        ) : (
              ...
              <tbody className="divide-y divide-[#EBE5DF]">
                {filteredCustomers.map((c) => (
                  <tr key={c.id} className="hover:bg-[#FAF6F0] transition-colors">
                    <td className="py-3.5 font-bold text-[#1C1917]">{c.name}</td>
